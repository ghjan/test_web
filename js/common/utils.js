 ...
 * author: Glen
 * created:　2015/5/29
 * last modified: zhangxinhe/2018/1/29
 */
...
define(['jquery'], function($) {
    ...
    var _addAttachment = function(app_label, model, id, atype) {
        if (app_label && model && id) {
            var attachment = $("#attachment");
            if (attachment.length > 0) {
                attachment = attachment.val();
                var value = app_label + "-" + model + "-" + id + "-" + atype;
                $("#attachment").val(attachment ? attachment + ',' + value : value);
            }
        }
    };

    window.addAttachment = function(app_label, model, id, atype) {
    ...
    return {
        ...
        checkContentUrl: function(content) {
            ...
            var pic_re = new RegExp(".+\.(png|jpg|bmp|gif)$");
        ...
        get_msg: function() {
            if ($.cookie('message')){
                layer.msg($.cookie("message"),{time: 1500});
                $.cookie("message",null);
            }
        },
        set_msg: function (msg) {
            var cookietime = new Date();
            cookietime.setTime(cookietime.getTime()+5000)
            $.cookie("message",msg,{expires:cookietime})
        },
		...
		load_template: function(template, data) {
			//alert(window.location);
			rsp = $.get(template, function(d){
				alert(data);
				return data;
			});
			//r =  rsp ? (data ? rsp(data): rsp): {error:'Template not found!'};
			//alert(rsp);
			return rsp;
		}
    };
